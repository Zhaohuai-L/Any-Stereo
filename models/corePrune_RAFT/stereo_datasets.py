...
class StereoDataset(data.Dataset):
    def __init__(self, aug_params=None, sparse=False, reader=None,multi_training=False,multi_input_training=False,scale_min=1,scale_max=4,inp_size=[132,240],without_mutli_scale=False):
        ...
        self.sample_q=inp_size[0]*inp_size[1]       # 132*236 = 31152
        self.is_test = False
    ...
    def __getitem__(self, index):
        ...
        if isinstance(disp, tuple):
            ...
            valid = disp < 512          # Why the say the valid is belong to 0~512, just in some speciall situations.
        ...
        if self.multi_training:         
            ...
            return self.image_list[index] + [self.disparity_list[index]],img1,img2,hr_coord,hr_flow.permute(1,0),arry_scale
...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None,multi_training=False,multi_input_training=False,scale_min=1,without_mutli_scale=False,scale_max=2,inp_size=[132,240],root='/mnt/FastData/', dstype='frames_finalpass', things_test=False): 
        super(SceneFlowDatasets, self).__init__(aug_params,multi_training=multi_training,multi_input_training=multi_input_training,scale_min=scale_min,scale_max=scale_max,without_mutli_scale=without_mutli_scale,inp_size=inp_size)                                                          
        self.root = root
        self.dstype = dstype
...
class KITTI12(StereoDataset):
    def __init__(self, aug_params=None, root='/mnt/Kitti/Kitti2012/stereoflow', image_set='training'):
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        if dataset_name.startswith("middlebury_"):
            ...
            final_dataset = SceneFlowDatasets(aug_params, multi_training=args.multi_training, multi_input_training=args.multi_input_training, scale_min=args.scale_min,
                                              scale_max=args.scale_max, inp_size=args.inp_size, without_mutli_scale=args.without_mutli_scale, dstype='frames_finalpass')
