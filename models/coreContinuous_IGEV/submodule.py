...
def context_upsample(disp_low, up_weights,scale):  # [B,1,H/4,W/4]  [B,9,H,W]  
    ###
    # cv (b,1,h,w)
    # sp (b,9,4*h,4*w)
    ###
    ...
    disp_unfold = F.interpolate(disp_unfold,(int(h*4*scale),int(w*4*scale)),mode='nearest').reshape(b,9,h*4,w*4)
...
def context_upsample_multiscale_train(disp_low, up_weights,hr_coord):  # [B,1,H/4,W/4]  [B,9,H,W]  
    ...
    disp_unfold = F.unfold(disp_low.reshape(b,c,h,w),3,1,1).reshape(b,-1,h,w)   #[B,9,H/4,W/4] 
