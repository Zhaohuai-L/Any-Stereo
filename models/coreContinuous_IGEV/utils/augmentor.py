...
class FlowAugmentorWoCrop:
    ...
    def __call__(self, img1, img2, flow,crop_size,scale_size):
        self.crop_size=crop_size
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, flow = self.spatial_transform(img1, img2, flow)
        #FIXME ？
        img1 = cv2.resize(img1, dsize=[scale_size[1],scale_size[0]], interpolation=cv2.INTER_CUBIC)
        img2 = cv2.resize(img2, dsize=[scale_size[1],scale_size[0]], interpolation=cv2.INTER_CUBIC)        
        img1 = np.ascontiguousarray(img1)
        img2 = np.ascontiguousarray(img2)
        flow = np.ascontiguousarray(flow)
...
class SparseFlowAugmentorWoCrop:
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=False, yjitter=False, saturation_range=[0.7,1.3], gamma=[1,1,1,1]):
        # spatial augmentation params
        self.crop_size = crop_size
        self.min_scale = min_scale
    ...
    def __call__(self, img1, img2, flow, valid,crop_size,scale_size):
        self.crop_size=crop_size
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, flow, valid = self.spatial_transform(img1, img2, flow, valid)
