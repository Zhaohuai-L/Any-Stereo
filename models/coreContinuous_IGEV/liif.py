...
def make_coord(shape, ranges=None, flatten=True):
    ...
    for i, n in enumerate(shape):
        if ranges is None:
            ...
            v0, v1 = ranges[i]
        r = (v1 - v0) / (2 * n)
        seq = v0 + r + (2 * r) * torch.arange(n).float()   # [(1-H)/H ~ (H-1)/H]  [(1-W)/W ~ (W-1)/W]
...
def liif_feat(feat, target_size, local=False,cell=False):
    ...
    for vx in vx_list:
        for vy in vy_list:
            ...
            q_feat = F.grid_sample(feat, coords_,mode='nearest',align_corners=False)[:,:,0,:].permute(0,2,1)
            q_coord = F.grid_sample(feat_coords, coords_,mode='nearest',align_corners=False)[:,:,0,:].permute(0,2,1)
    ...
    if not local:
        ...
        return rel_coord_list, q_feat_list, area_list,rel_cell_list       # [4*[B,H*W,2]]  [4*[B,H*W,C]]  [4*[B,H*W]]  
...
def liif_feat_multiscale_train_quater(feat, coords,scale, local=False,cell=False):
    ...
    for vx in vx_lst:
        for vy in vy_lst:
            ...
            coords_[:,:,1] += vy * ry + eps_shift
            coords_.clamp_(-1+1e-6, 1-1e-6)
            coords_=coords_.flip(-1).unsqueeze(1) 
...
class SpatialEncoding(nn.Module):
    def __init__(self,
        ...
        m = np.concatenate([np.roll(m, i, axis=-1)
                           for i in range(in_dim)], axis=0)
...
class liif_out(nn.Module):
    def __init__(self, pos_dim=24, encoder_dim=256, mlphidden_list=[128, 64, 64], pos_enconding=False, local_ensemble=False, decode_cell=False, unfold=False, require_grad=True):
        ...
        self.pos_encoding = SpatialEncoding(
    ...
    def forward(self, feat, tar_size):
        h_h, h_w = tar_size
        b, c, h, w = feat.shape
        if not self.local_ensemble:
            ...
            rel_coord, q_feat, rel_cell = liif_feat(
                feat, tar_size, self.local_ensemble, self.decode_cell)        # [B,H*W,2]  [B,H*W,C]
            if self.pos_enconding:
            ...
            if self.unfold:
                feat = F.unfold(feat, 3, padding=1).view(
                    feat.shape[0], feat.shape[1] * 9, feat.shape[2], feat.shape[3])  # [B,9C,H/4,W/4]
            rel_coord_list, q_feat_list, area_list, rel_cell_list = liif_feat(
                feat, tar_size, self.local_ensemble, self.decode_cell)    # [4*[B,H*W,2]]  [4*[B,H*W,C]]  [4*[B,H*W]]
            preds = []
            for q_feat, rel_coord, rel_cell in zip(q_feat_list, rel_coord_list, rel_cell_list):
                if self.pos_enconding:
                    rel_coord = self.pos_encoding(
                        rel_coord)  # Position Encoding
            ...
            tot_area = torch.stack(area_list).sum(dim=0)
            t = area_list[0]
    ...
    def forward(self, feats, coord,scale):      #[B,80,H/4,W/4]   [H,W]
        ...
        for feat in feats:
            ...
            if self.quater_nearest!=None:
                ...
                rel_coord, q_feat,rel_cell=liif_feat_multiscale_train(feat,coord,scale,self.local_ensemble,self.decode_cell)        # [B,H*W,2]  [B,H*W,C]        
            if self.pos_enconding: 
                rel_coord = self.pos_encoding(rel_coord) 
            elif self.pos_enconding_new:
                _,rel_coord = self.pos_encoding(rel_coord) 
            feat=torch.cat([q_feat,rel_coord],dim=-1)    #[B,H*W,C]
            del rel_coord,q_feat
            if self.decode_cell:
                feat=torch.cat([feat,rel_cell],dim=-1)    #[B,H*W,C]
        ...
        latent=latent.permute(0,2,1) #[B,C,H*W]   
