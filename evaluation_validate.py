...
from models.corePrune_RAFT.prune_raft_stereo import  autocast
import models.corePrune_RAFT.stereo_datasets as datasets
import models.corePrune_RAFT.stereo_datasets as datasetsSF
import models.corePrune_RAFT.stereo_datasets as datasetsMix
...
def Disp_to_color(Disp,max_disp=192):
    ...
    output=map[ind,:3]* (1-Dispout).unsqueeze(3).repeat(1,1,1,3)+ map[ind+1,:3] * (Dispout).unsqueeze(3).repeat(1,1,1,3)
...
def pad_for_multi_train(args,image1,image2):
    ...
    image1_pad, image2_pad = padder.pad(image1, image2)  #192 624  
    pad_num=padder.get_pad_num()   
    h_hr_pad=(image1_pad.shape[2])*args.scale_test     
    w_hr_pad=(image1_pad.shape[3])*args.scale_test    #384 1248
    hr_pad_coord=make_coord([h_hr_pad,w_hr_pad],flatten=False)   
    if args.scale_test>1:
        pad_num=[round(i*args.scale_test) for i in pad_num] 
...
def pad_for_multi_train_Fixed(scale,image1,image2):
    h_want, w_want = [image1.shape[-2]*scale,image1.shape[-1]*scale] 
    padder = InputPadder(image1.shape, divis_by=16) 
    image1_pad, image2_pad = padder.pad(image1, image2)  #192 624  
    pad_num=padder.get_pad_num()   
    h_hr_pad=(image1_pad.shape[2])*scale    
    w_hr_pad=(image1_pad.shape[3])*scale    #384 1248
    hr_pad_coord=make_coord([h_hr_pad,w_hr_pad],flatten=False)   
    pad_num=[round(i*scale) for i in pad_num] 
...
def pad_for_muti_other(args,image1,image2):
    ...
    w_lr=math.ceil(w_want/args.scale_test)      # 621
    if args.scale_test>1:
        image1=F.interpolate(image1,(h_lr,w_lr),mode='bicubic',align_corners=False) 
        image2=F.interpolate(image2,(h_lr,w_lr),mode='bicubic',align_corners=False)
    padder = InputPadder(image1.shape, divis_by=32)
...
@torch.no_grad()
def validate_kitti(args,logger,model, iters=32, mixed_prec=False,mode=None):
    ...
    avg_test_scalars.clean(),avg_test_scalars_occ.clean(),avg_test_scalars_nonocc.clean()    
    if mode=='kitti15':
        val_dataset = datasetsMix.KITTImixed(aug_params= {}, mode='valid_15')
    elif mode=='kitti12':
        val_dataset = datasetsMix.KITTImixed(aug_params= {}, mode='valid_12')
    else : assert False
    torch.backends.cudnn.benchmark = True
    for val_id in range(len(val_dataset)):
        (imageL_file, imageR_file, GT_file), image1, image2, flow_gt = val_dataset[val_id]
        ...
        if args.model=='RAFTStereo': 
            ...
            with autocast(enabled=mixed_prec):
                if args.multi_training or args.multi_input_training:
                    start = time.time()
                    flow_pr = model(image1_pad, image2_pad, iters=iters, hr_coord=hr_coord.unsqueeze(0).cuda(),scale=torch.tensor([args.scale_test]).view(1,1).cuda(),test_mode=True)
                    end = time.time()
                    flow_pr=flow_pr.view(1,image1.shape[2],image1.shape[3]).cpu();disp=flow_pr; disp_gt=flow_gt
                elif args.multi_evaothers:
                    start = time.time()
        ...
        valid_mask=(disp_gt>0)&(disp_gt<1000) 
        if args.max_enable:
            valid_mask=valid_mask&(disp_gt<args.max_disp)
...
def validate_things(args,logger,model, iters=32, mixed_prec=False):
    ...
    for val_id , data in enumerate(testbar):
        ...
        if args.output:
            ...
            save_image(color_disp,os.path.join(args.savepath,"output","disp_{}.png".format(name)))
        testbar.set_description("EPE = {:.3f}".format(scalar_outputs["EPE"][0]))            
        avg_test_scalars.update(tensor2float(scalar_outputs))
        avg_test_scalars_occ.update(tensor2float(scalar_outputs_occ))
        avg_test_scalars_nonocc.update(tensor2float(scalar_outputs_nonocc))      
        save_scalars(logger, 'Test', scalar_outputs, val_id)
...
def validate_middlebury(args,logger,model, iters=32, split='F', mixed_prec=False, Before_GRU=False):
    ...
    for val_id in range(len(val_dataset)):
        (imageL_file, _, disparity_file), image1, image2, flow_gt, valid_gt = val_dataset[val_id]
        name=imageL_file.split('/')[-2]; image1 = image1[None].cuda(); image2 = image2[None].cuda()
        if args.multi_training or args.multi_input_training:
            if split=='Q_F':
                image1_pad,image2_pad,hr_coord=pad_for_multi_train_Fixed(4,image1,image2)
            elif split=='H_F':
                image1_pad,image2_pad,hr_coord=pad_for_multi_train_Fixed(2,image1,image2)
            else:
                image1_pad,image2_pad,hr_coord=pad_for_multi_train(args,image1,image2)
        ...
        if args.model=='RAFTStereo': 
            with autocast(enabled=mixed_prec):
                _, flow_pr = model(image1_pad, image2_pad, iters=iters, test_mode=True)
            flow_pr = padder.unpad(flow_pr).cpu().squeeze(0); disp=-flow_pr; disp_gt=flow_gt
        elif "UACorreStereo" in args.model: 
            with autocast(enabled=mixed_prec):
                _,flow_pr = model(image1_pad, image2_pad, iters=iters, test_mode=True,train_noc_only=Before_GRU)
            flow_pr = padder.unpad(flow_pr).cpu().squeeze(0); disp=flow_pr; disp_gt=flow_gt        
        else: 
            with autocast(enabled=mixed_prec):
                if args.multi_training or args.multi_input_training:
                    if split=='Q_F':
                        ...
                        _scale=args.scale_test
                    flow_pr = model(image1_pad, image2_pad, iters=iters, hr_coord=hr_coord.unsqueeze(0).cuda(),scale=torch.tensor([_scale]).view(1,1).cuda(),test_mode=True)
                    if split=='Q_F' or split=='H_F' :
                        flow_pr=flow_pr.view(1,image1.shape[2]*_scale,image1.shape[3]*_scale).cpu();disp=flow_pr; disp_gt=flow_gt
                    else:
                        flow_pr=flow_pr.view(1,image1.shape[2],image1.shape[3]).cpu();disp=flow_pr; disp_gt=flow_gt
                elif args.multi_evaothers:
                    flow_pr = model(image1_pad, image2_pad, iters=iters, test_mode=True)
        ...
        assert flow_pr.shape == flow_gt.shape, (flow_pr.shape, flow_gt.shape)
        disp = torch.where(torch.isinf(disp), torch.full_like(disp, 0), disp)
        occ_mask = Image.open(disparity_file.replace('disp0GT.pfm', 'mask0nocc.png')).convert('L')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pos_dim', type=int, default=12, help="width of the correlation pyramid")
    parser.add_argument('--mlphidden_list', type=int, nargs='+', default=[64, 32], help="size of the random image crops used during training.")
    ...
    parser.add_argument('--agg_type', default=None, help='')  
